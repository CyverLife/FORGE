...
BEGIN
    ...
    SELECT attribute INTO habit_essence -- Changed because 'essence_type' does not exist in schema, 'attribute' does
    FROM habits
    ...
    IF NEW.status = 'completed' THEN
        ...
        SET
            ...
            angel_score = CASE WHEN habit_essence IN ('IRON', 'FIRE') THEN COALESCE(angel_score, 0) + 1 ELSE angel_score END, -- Mapped to available attributes
            simio_score = CASE WHEN habit_essence IN ('STEEL', 'FOCUS') THEN COALESCE(simio_score, 0) + 1 ELSE simio_score END,
            updated_at = NOW()
